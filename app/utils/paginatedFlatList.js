...
class PaginatedFlatList extends Component {
    ...
    constructor(props){
        ...
        this.tryDownloadNewElementsDebounce = debounce(this.tryDownloadNewElements, 500, {leading: true, trailing: false});
    }
    ...
    getOtherElements = () => {
        // console.log("PaginatedFlatList getOtherElements page:"+this.lastRequestedPage+"++");
        ...
        fetchFromServer(
            ...
            // console.log(`PaginatedFlatList getOtherElements()`, response);
            if(!this.isComponentMounted) return;
            if (response.status < 300) {
                ...
                newData.push(...data);
                // Allow to edit the data outside this component
                let editedData = this.letDataBeEdited(newData);

                // Signal that the download is over
                this.setState({list: editedData, loadingList: false});
            }
            else
            {
...
export default connect(mapStateToProps, mapDispatchToProps, null, { forwardRef: true })(PaginatedFlatList);